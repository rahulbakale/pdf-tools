...
class DecryptPdfsServiceTest {
  ...
  void test_decryptPdfs() throws IOException {
    ...
    outputDirContentsRelativePaths.stream()
  ...
  private static void assertPdfsEqual(Path path, Path expectedPdfsDirPath, Path actualPdfsDirPath) {
    ...
    Assertions.assertTrue(
        PdfUtils.pdfEquals(expectedPdfPath, actualPdfPath),
        String.format(
            "The following PDFs are not equal: '%s', '%s'", expectedPdfPath, actualPdfPath));
  }
